...
def clean_price(value):
    ...
    try:
        return float(value.replace('n', ''))  # Convert the numeric part of the value
    except ValueError:
        return np.nan  # If it can't be converted, return NaN

# Apply the clean_price function to the 'price' column
df_numeric_properties['cleaned_value'] = df_numeric_properties['value'].apply(clean_price)
...
df_clean_prices['cleaned_value'] = df_clean_prices['cleaned_value'].astype(float)
...
df_pivot.rename(columns={
    ...
    'available': 'stock'       # Assuming 'available' represents stock status (1 = in stock, 0 = out of stock)
}, inplace=True)

# Step 8: Handle missing data
df_pivot['price'] = df_clean_prices['cleaned_value'].fillna(0).astype(float)  # Use cleaned prices or default to 0
