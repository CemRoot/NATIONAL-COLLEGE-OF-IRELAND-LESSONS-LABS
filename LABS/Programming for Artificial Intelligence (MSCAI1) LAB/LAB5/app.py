...
def load_user(user_id):
    ...
    if user:
        return User(
            user_id=user[0], username=user[1], email=user[3], password_hash=user[2]
        )
    return None
...
def add_to_cart(product_id):
    ...
    cur = mysql.connection.cursor()
    cur.execute(
        "SELECT * FROM OrderItems WHERE user_id = %s AND product_id = %s",
        (user_id, product_id),
    )
    item = cur.fetchone()
    ...
    if item:
        cur.execute(
            "UPDATE OrderItems SET quantity = quantity + %s WHERE user_id = %s AND product_id = %s",
            (quantity, user_id, product_id),
        )
    else:
        cur.execute(
            "INSERT INTO OrderItems (user_id, product_id, quantity) VALUES (%s, %s, %s)",
...
def checkout():
    ...
    if request.method == "POST":
        cur.execute(
            "INSERT INTO Orders (user_id, total_amount, order_date) VALUES (%s, %s, NOW())",
            (user_id, total),
        )
        ...
        for item in cart_items:
            ...
            cur.execute(
                "DELETE FROM OrderItems WHERE user_id = %s AND product_id = %s",
                (user_id, item[0]),
            )
